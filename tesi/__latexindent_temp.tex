...
Inizialmente sono stati eseguiti degli esperimenti per capire come i parametri del modello influissero sulla predizione dell'algoritmo. Nel corso di tutti gli esperimenti è stato utilizzato il kernel Gaussiano, di conseguenza tra i parametri del modello è stato considerato anche $\sigma$. Nelle Figure \ref{fig:Setosa_Preliminari}-\ref{fig:Versicolor_Preliminari}-\ref{fig:Virginica_Preliminari} vengono mostrati i risultati di questi primi esperimenti. Nell'asse delle ascisse il numero di iterazioni effettuate utilizzanto il solver implementato con TensorFlow; nell'asse delle ordinate, invece, c'è la radice dell'errore quadratico medio (Root Mean Square Error)
...
% Figura Tabella Parametri (c, sigma, Pen) x 3 Classi (RMSE)
% Commenti
...
% Figura Tabella stessi parametri con Penalization diversa
% Commenti
